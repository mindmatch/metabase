...
;;
;; TODO: this doesn't work on MySQL. It relies on the JDBC prepared statement stuff that is broken in the MySQL
;; driver. On MBQL queries, we convert it to a string, then use the MySQL `convert_tz` function to work around the
;; issue. Maybe we should be doing the same thing for native queries. Specifically now we're calling Honeysql's format
;; function on the date directly and including that. Maybe it should go through the same code that MBQL goes through
;; (i.e. sqlqp->honeysql)
(expect-with-engines [:postgres #_:mysql]
  #{[6 "Shad Ferdynand" "2014-08-02T05:30:00.000-07:00"]
    [7 "Conchúr Tihomir" "2014-08-02T02:30:00.000-07:00"]}
  (with-tz-db
    (tu/with-temporary-setting-values [report-timezone "America/Los_Angeles"]
       ...
       (qpt/rows
         (qp/process-query
           {:database (data/id)
            ...
            :parameters [{:type "date/single" :target ["variable" ["template-tag" "date1"]] :value "2014-08-02T03:00:00.000000"}
                         {:type "date/single" :target ["variable" ["template-tag" "date2"]] :value "2014-08-02T06:00:00.000000"}]}))))))

;; This does not currently work for PostgreSQL or MySQL
;;
;; The issue with MySQL looks to be the same as the one above. The issue with PostgreSQL looks to be related to how
;; the parameters are being added to the prepared statement for variables. The SQL query looks fine and the date/time
;; looks identical to the working PG query above.
#_(expect-with-engines [:postgres :mysql]
  #{[6 "Shad Ferdynand" "2014-08-02T05:30:00.000-07:00"]
    [7 "Conchúr Tihomir" "2014-08-02T02:30:00.000-07:00"]}
  (with-tz-db
    (tu/with-temporary-setting-values [report-timezone "America/Los_Angeles"]
       ...
       (qpt/rows
         (qp/process-query
           {:database (data/id)
            ...
            :parameters [{:type "date/range", :target ["dimension" ["template-tag" "ts_range"]], :value "2016-08-02~2016-08-03"}]}))))))

;; This is broken for MySQL in the same way it's broken in the above two queries. There's two problems, the first is
;; we're comparing a date with a timestamp, we should be casting to a date. The second is related to the same issue
;; the above two queries have, that the timestamp isn't being adjusted for the timezone correctly. Even if we fixed
;; the date casting, we could still run into issues where the timezone offset crosses the date boundaries. This
;; reproduces the bug found in https://github.com/metabase/metabase/issues/7393
(expect-with-engines [:postgres #_:mysql]
  #{[6 "Shad Ferdynand" "2014-08-02T05:30:00.000-07:00"]
    [7 "Conchúr Tihomir" "2014-08-02T02:30:00.000-07:00"]}
  (with-tz-db
